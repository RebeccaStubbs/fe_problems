...
## Exploring a single analyte as a test case
...
As part of the exercise, I need to generate a "time series by station and analyte". Before looping over each station-analyte combination, I need to create a template graphic that I'm happy with. Below is code to graph a subset of the data (one station's measurements for a specific analyte), with the data symbolized by season (in this case, using color to clarify which season the data point was taken in). 
...
# Make and print plot
  ts<-MakeAnalyteTSPlot(a="Temperature",s="FE-4023") # Arbitrary station chosen
  print(ts)
```

OK, good enough to iterate over (not knowing the audience of these graphs, and what level of polish the client/we are looking for internally). We can now make an interactive plot version of this as an R shiny, below: 
...
```
We can also save these as PDFs (although that's not nearly as fun). 
