...
for (chemical in names(chemlist)){ # For each chem
  ...
  p1<-ggplot() +  
    ...
    guides(fill=guide_colourbar(title=paste0("log-transformed /n",chem_info[["unit"]]), 
...
To understand how 
```{r}
# Calculate a data set by decade
...
chem_by_decade_rm<-chem_data[!is.na(chem) & Detected=="Yes" &
                            ...
                            q1=quantile(observed_value,.75),
                            Unit),
by=c("chem","Year","RM_up","Depth","Decade")]
```
...
for(chemical in names(chemlist)){
 ...
 # Make plot of highest values
  p<-ggplot() + 
  ggtitle(chemical, subtitle="Mean obsered values, across all sediment depths")+
        # Rectangles to simulate core depths and values
         geom_point(data=chem_simplified[chem==chemical], 
                  aes(x=RM_up,y=log(obs_mean), color=Year)) + # Color by log-value
...
As a first pass, however, I can still make a map of unique sampled locations, and when (if at all) these samples were dredged:
